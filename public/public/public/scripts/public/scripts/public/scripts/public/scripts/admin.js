import { db, machinesCol, bookingsCol, auditCol, fs } from './firebase.js';
...
function renderLogs(entries) {
  ...
  entries.forEach(e => {
    ...
    const btnDel = document.createElement('button'); btnDel.textContent = 'Delete'; btnDel.onclick = () => fs.deleteDoc(bookingsCol.doc(e.id));
    const btnComp = document.createElement('button'); btnComp.textContent = 'Complete'; btnComp.onclick = () => fs.updateDoc(bookingsCol.doc(e.id), { completed: true, completedTime: new Date() });
    const btnPdf = document.createElement('button'); btnPdf.textContent = 'Export PDF'; btnPdf.onclick = () => window.print();
